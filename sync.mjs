...
function suggestMoveFromGitHub(targetRoot) {
  ...
  console.log('💡 .github/ cleanup suggestion\n');
  console.log(
    '   You can delete all agents and skills from .github/ in your fork.',
  );
  console.log(
    '   Copilot auto-discovers from .github/, but sync populates it for',
  );
  console.log(
    '   consuming repos — in YOUR fork, root agents/ and skills/ are the',
  );
  console.log('   source of truth.\n');
  console.log(
    '   Exception: keep custom agents/skills in .github/ ONLY if you want',
  );
  console.log(
    '   Copilot to auto-detect them when working directly in this fork',
  );
  console.log(
    "   (e.g. fork-specific agents you don't distribute to consumers).\n",
  );

  if (hasDuplicates) {
    console.log("   Safe to delete (duplicates of what's already in root):");
    if (duplicateAgents.length) {
      duplicateAgents.forEach((f) =>
        console.log(`     rm .github/agents/${f}`),
      );
    }
    if (duplicateSkills.length) {
      duplicateSkills.forEach((s) =>
        console.log(`     rm -rf .github/skills/${s}/`),
      );
    }
    console.log('');
  }
  ...
  if (hasOrphans) {
    console.log(
      '   Custom content found only in .github/ — move to root to distribute,',
    );
    console.log(
      '   or leave in .github/ for fork-local Copilot discovery only:',
    );
    if (orphanAgents.length) {
      orphanAgents.forEach((f) =>
        console.log(`     mv .github/agents/${f} agents/${f}  # to distribute`),
      );
    }
    if (orphanSkills.length) {
      orphanSkills.forEach((s) =>
        console.log(`     mv .github/skills/${s} skills/${s}  # to distribute`),
      );
    }
    console.log('');
  }

  console.log('   After cleanup, re-run sync if needed.\n');
}
